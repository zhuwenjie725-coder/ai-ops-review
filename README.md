...
Codex CLI 用于辅助代码开发与调试；工具本身由 Python、pandas、openpyxl、matplotlib 等在本地运行，不调用外部 AI 模型，不使用网页或数据库。所有示例数据均为模拟测试数据。
...
优先展示 `dashboard.png`；补充截取 Excel 的“数据质量报告”“达人分层建议”中带有触发规则的行，以及“参数配置说明”。图表和阈值只是演示，不应描述为真实投放决策标准。
